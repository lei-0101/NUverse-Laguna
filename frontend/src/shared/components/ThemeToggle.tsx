...
export function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="relative flex h-10 w-10 items-center justify-center rounded-xl text-foreground transition-all duration-200 hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary active:scale-90"
      style={bursting ? { animation: 'sun-burst 0.35s ease-out' } : undefined}
    >
      {isDark ? (
        /* Moon (dark mode) — click to go light */
        <span className="theme-toggle-moon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            />
            {/* Three tiny stars orbiting the moon */}
        ...
        /* Sun (light mode) — click to go dark */
        <span className="theme-toggle-sun group" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg viewBox="0 0 24 24" className="h-5 w-5 text-amber-500" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Sun core */}
            <circle cx="12" cy="12" r="4.5" fill="currentColor" className="sun-icon" />
            {/* 8 rays */}
