...
-- product_variants columns: id, product_id, size, color, sku, stock, price, created_at, updated_at
-- merchandise_products uses is_active (not active) and limited
...
BEGIN
  ...
  FOR v IN
    SELECT * FROM product_variants WHERE size IN ('XS–3XL', 'XS-3XL')
  LOOP
    sizes    := std_sizes;
    sku_base := regexp_replace(v.sku, '[-_]?(STD|XS.*3XL)$', '', 'i');
    FOREACH s IN ARRAY sizes LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (
        gen_random_uuid(), v.product_id, s, v.color,
        sku_base || '-' || s,
        GREATEST(v.stock / array_length(sizes, 1), 2),
        v.price, v.created_at, v.updated_at
      );
    END LOOP;
...
-- ── Update SHS Traditional Uniform products to SHS category ──────────────
UPDATE merchandise_products
SET category = 'SHS'
WHERE name LIKE '[SHS]%'
  AND category = 'CLOTHING';

-- ── Add College Traditional Uniform products (idempotent) ────────────────
DO $$
DECLARE
  prod_id UUID;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM merchandise_products WHERE name = '[College] Traditional Uniform Male Polo') THEN
    ...
    -- Male Polo
    INSERT INTO merchandise_products (id, name, description, image_url, category, gender, is_active, limited, created_at, updated_at)
    ...
    FOREACH s IN ARRAY std_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, NULL, 'TRAD-COL-M-T-' || s, 3, 650.00, NOW(), NOW());
    END LOOP;
    FOREACH s IN ARRAY ext_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, NULL, 'TRAD-COL-M-T-' || s, 1, 950.00, NOW(), NOW());
    END LOOP;
    ...
    FOREACH s IN ARRAY std_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, 'Navy Blue', 'TRAD-COL-M-B-' || s, 3, 550.00, NOW(), NOW());
    END LOOP;
    FOREACH s IN ARRAY ext_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, 'Navy Blue', 'TRAD-COL-M-B-' || s, 1, 800.00, NOW(), NOW());
    END LOOP;
    ...
    FOREACH s IN ARRAY std_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, NULL, 'TRAD-COL-F-T-' || s, 3, 650.00, NOW(), NOW());
    END LOOP;
    FOREACH s IN ARRAY ext_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, NULL, 'TRAD-COL-F-T-' || s, 1, 950.00, NOW(), NOW());
    END LOOP;
    ...
    FOREACH s IN ARRAY std_sz LOOP
      INSERT INTO product_variants (id, product_id, size, color, sku, stock, price, created_at, updated_at)
      VALUES (gen_random_uuid(), prod_id, s, 'Navy Blue', 'TRAD-COL-F-B-' || s, 3, 400.00, NOW(), NOW());
    END LOOP;
